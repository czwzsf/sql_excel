...
class sale_data_test_form(forms.ModelForm):
    class Meta:
        model = models.sale_data_tset
        fields = "__all__"
...
def upload_excel_sale_test(request):
    """基于Excel的文件上传"""
    # 1. 获取用户上传的销售数据文件对象
    form = sale_data_test_form(data=request.POST, files=request.FILES)
    file_object = request.FILES.get("filename", None)
    if not file_object:
        return HttpResponse("没有文件可供上传")
    # 2. 将文件对象传递给openpyxl，由openpyxl来进行读取
    ...
    for row in sheet.iter_rows(min_row=2):
        for item in range(len(row)):
            text = str(row[item].value)
            dict_list.append(text)
        models.sale_data_tset.objects.create(production_code=dict_list[1], chassis_code=dict_list[2],
                                             offline_data=dict_list[3],
                                             sale_data=dict_list[4], engine_number=dict_list[5],
                                             engine_type=dict_list[6],
                                             responsible_organization=dict_list[7], dealer_code=dict_list[8],
                                             dealer_name=dict_list[9], representative_office_code=dict_list[10],
                                             representative_office_name=dict_list[11], cluth=dict_list[12],
                                             gear_box=dict_list[13],
